...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  let currentFFN = {nHidden:2,positions:null};
  ...
  function runForward(animate=true){
    ...
    if(animate){
      const path = [[100,60],[200,90],[380,140],[600,140]]; Visuals.animateBall(svg,path,{duration:1200});
    }
    // expose last values
  ...
  function buildFFNControls(nHidden){
    ...
    // hidden layer weights: for each hidden neuron, bias + weights for two base inputs
  ...
  function computeFFN(){
    ...
    let feats = [x1v, x2v];
    if(feat_x2.checked){ feats.push(x1v*x1v); }
    if(feat_sqrt.checked){ feats.push(Math.sqrt(Math.abs(x1v))); }
    if(feat_inter.checked){ feats.push(x1v*x2v); }
    // for now only use first two raw features for weights; extension possible
    const hidden = [];
    for(let j=0;j<nHidden;j++){
    ...
    // render LaTeX: hidden neurons and output
    let tex = '';
    hidden.forEach((h,i)=>{
    ...
    window._dt_state.ffn = {hidden, b_o, v, yhat, feats};
  }

  hiddenSizeSel.addEventListener('change', ()=>{
    const n = parseInt(hiddenSizeSel.value,10); buildFFNControls(n); currentFFN.nHidden = n; currentFFN.positions = Visuals.drawFFN(svg,n,2); computeFFN();
  });
  ...
  // initialize FFN controls
  buildFFNControls(parseInt(hiddenSizeSel.value,10)); currentFFN.positions = Visuals.drawFFN(svg, parseInt(hiddenSizeSel.value,10), 2);
  computeFFN();
  ...
  const challenges = [
    ...
    { id: 'easy_linear', title:'Linear: simple coeffs', difficulty:'easy',
      desc:'Hidden size 0. Set weights so that \(\\hat{y} = 2 x_1 + 1 x_2 + 0\\).\nUse small integers.',
    ...
    { id: 'easy_match', title:'Match batting probability', difficulty:'easy',
      ...
      preset:{x1:92,x2:25,hiddenSize:1,activation:'identity'},
      solution:{ b:[0], w1:[0.01], w2:[0.01], v:[0.01], b_o:0 }, tol:0.1,
    ...
    { id: 'med_recreate', title:'Recreate a small FFN mapping', difficulty:'medium',
      ...
      preset:{x1:10,x2:3,hiddenSize:2,activation:'identity'},
      // a sample solution that yields 50: choose hidden to replicate basis
      solution:{ b:[0,0], w1:[2,3], w2:[0,1], v:[1,2], b_o:0 }, tol:1.0,
    ...
    { id: 'hard_challenge', title:'Design a small classifier', difficulty:'hard',
      desc:'Hidden size 2. Using sigmoid activations, design weights that produce ŷ>0.8 for positive examples and <0.2 for negatives (manual rule). See hints.',
    ...
    { id: 'hard_fit', title:'Fit target output', difficulty:'hard',
      ...
      preset:{x1:12,x2:4,hiddenSize:3,activation:'identity'},
      solution:{ b:[0,0,0], w1:[2,2,2], w2:[0,0,0], v:[10,20,1], b_o:3 }, tol:2.0,
  ...
  function populateChallenges(){
    ...
    const sel = difficultySel.value || 'all';
    ffnChallenge.innerHTML = '<option value="none">-- Select a challenge --</option>';
    challenges.filter(c=> sel==='all' ? true : c.difficulty===sel ).forEach(ch=>{ const o=document.createElement('option'); o.value=ch.id; o.textContent=`[${ch.difficulty}] ${ch.title}`; ffnChallenge.appendChild(o) });
  }

  function loadChallenge(id){
    const ch = challenges.find(c=>c.id===id); if(!ch) return; challengeDesc.innerHTML = ch.desc;
    // reset hint state
    hintIndex = 0; hintBox.textContent = '';
    // apply preset
    if(ch.preset){
      el('x1').value = ch.preset.x1; el('x1num').value = ch.preset.x1; el('x2').value = ch.preset.x2; el('x2num').value = ch.preset.x2;
      hiddenSizeSel.value = ch.preset.hiddenSize; hiddenSizeSel.dispatchEvent(new Event('change'));
      el('activation').value = ch.preset.activation || 'identity';
      computeFFN();
    }
    challengeFeedback.textContent = '';
  }

  ffnChallenge.addEventListener('change', ()=>{ loadChallenge(ffnChallenge.value); });
  difficultySel.addEventListener('change', ()=>{ populateChallenges(); challengeFeedback.textContent=''; });

  checkSolutionBtn.addEventListener('click', ()=>{
    const id = ffnChallenge.value; const ch = challenges.find(c=>c.id===id);
    if(!ch){ challengeFeedback.textContent = 'Select a challenge first.'; return; }
    computeFFN();
    const state = window._dt_state.ffn;
    if(ch.preset.hiddenSize === 0){
      // compare linear weights w0,w1,w2
      const w0 = parseFloat(el('w0').value), w1=parseFloat(el('w1').value), w2=parseFloat(el('w2').value);
      const targetFn = ch.solution; const x1v = parseFloat(el('x1').value), x2v = parseFloat(el('x2').value);
      const yhat = w0 + w1*x1v + w2*x2v; const ytarget = targetFn.w0 + targetFn.w1*x1v + targetFn.w2*x2v;
      const ok = Math.abs(yhat-ytarget) <= ch.tol;
      challengeFeedback.textContent = ok ? `Success — ŷ=${Utils.fmt(yhat)} matches target ${Utils.fmt(ytarget)} within tol ${ch.tol}` : `Not yet — ŷ=${Utils.fmt(yhat)} target ${Utils.fmt(ytarget)} (diff ${Utils.fmt(yhat-ytarget)})`;
    } else {
      const yhat = state.yhat; const target = ch.solution && ch.solution.targetY ? ch.solution.targetY : null;
      // if solution provided as weights, compute target by applying solution to preset inputs
      let targetY = null;
      if(ch.solution && ch.solution.v){
        // compute using supplied solution
        const x1v = parseFloat(el('x1').value), x2v = parseFloat(el('x2').value);
        const hiddenActs = ch.solution.b.map((b,j)=>{ return b + ch.solution.w1[j]*x1v + ch.solution.w2[j]*x2v });
        const ysol = ch.solution.b_o + hiddenActs.reduce((s,h,i)=> s + ch.solution.v[i]*h, 0);
        targetY = ysol;
      }
      if(targetY===null) targetY = 1.0; // fallback
      const ok = Math.abs(yhat - targetY) <= ch.tol;
      challengeFeedback.textContent = ok ? `Success — ŷ=${Utils.fmt(yhat)} matches challenge target ${Utils.fmt(targetY)} within ${ch.tol}` : `Not yet — ŷ=${Utils.fmt(yhat)} target ${Utils.fmt(targetY)} (diff ${Utils.fmt(yhat-targetY)})`;
    }
  });

  showSolutionBtn.addEventListener('click', ()=>{
    const id = ffnChallenge.value; const ch = challenges.find(c=>c.id===id); if(!ch){ challengeFeedback.textContent='Select a challenge first.'; return; }
    if(ch.preset.hiddenSize===0 && ch.solution){ el('w0').value = ch.solution.w0; el('w1').value = ch.solution.w1; el('w2').value = ch.solution.w2; challengeFeedback.textContent='Solution applied to weights.'; }
    else if(ch.solution && ch.solution.v){
      // apply hidden and output weights
      hiddenSizeSel.value = ch.preset.hiddenSize; hiddenSizeSel.dispatchEvent(new Event('change'));
      // set hidden weights
      ch.solution.b.forEach((bv,j)=>{ const inb = ffnControls.querySelector(`input[data-role='b'][data-j='${j}']`); if(inb) inb.value = bv; const w1i = ffnControls.querySelector(`input[data-role='w'][data-j='${j}'][data-i='1']`); if(w1i) w1i.value = ch.solution.w1[j]; const w2i = ffnControls.querySelector(`input[data-role='w'][data-j='${j}'][data-i='2']`); if(w2i) w2i.value = ch.solution.w2[j]; });
      const ob = ffnControls.querySelector('#out_b'); if(ob) ob.value = ch.solution.b_o; ch.solution.v.forEach((vv,j)=>{ const vi = ffnControls.querySelector(`input[data-role='v'][data-j='${j}']`); if(vi) vi.value = vv; });
      computeFFN(); challengeFeedback.textContent = 'Solution weights applied.';
    } else { challengeFeedback.textContent = 'No solution is available for this challenge.'; }
  });

  populateChallenges();

  // --- hint UI ---
  const hintBtn = el('hintBtn'); const nextHintBtn = el('nextHintBtn'); const hintBox = el('hintBox'); let hintIndex = 0;
  ...
  function computeGrad(){
    ...
    const pred = st.a;
    // for single-sample linear regression with activation, include activation derivative
  ...
  function logEvent(msg){ const t=new Date().toLocaleTimeString(); log.textContent = `[${t}] ${msg}\n` + log.textContent }

  // keyboard shortcuts
  document.addEventListener('keydown', e=>{
    if(e.code==='Space'){ e.preventDefault(); el('runForward').click(); }
  ...
  window.DiamondTrainer = { runForward, computeGrad, applyUpdate, randomizeWeights };
});
